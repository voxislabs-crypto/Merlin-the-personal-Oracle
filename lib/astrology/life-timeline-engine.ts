...
import { findSolarArcPeakMarkers } from '@/lib/astrology/solar-arc';
...
export interface TimelineStrike {
  ...
  source?: 'transit' | 'solar-arc';
}

export interface LifeTimeline {
  events: TimelineStrike[];
  solarArcPeaks: TimelineStrike[];
  birthYear: number;
  currentAge: number;
  birthDate: string;
}
...
export async function buildLifeTimeline(
  ...
  const ascendant = natalChart.find((planet) => planet.name.toLowerCase() === 'ascendant');
  const midheaven = natalChart.find((planet) => planet.name.toLowerCase() === 'midheaven');
  const solarArcPeaks = findSolarArcPeakMarkers({
    natalPlanets: natalChart.filter(
      (planet) => !['ascendant', 'midheaven'].includes(planet.name.toLowerCase())
    ),
    birthYear,
    ...
    month: 7,
    transitingPlanet: peak.directedPlanet.toLowerCase(),
    natalPlanet: peak.natalPlanet.toLowerCase(),
    aspect: peak.aspect.toLowerCase(),
    orb: peak.orb,
    exactDate: `${peak.year}-07-01`,
    oneLiner: peak.oneLiner,
    intensity: peak.orb <= 0.15 ? 'strike' as const : 'shift' as const,
    raw: `SA ${peak.directedPlanet} ${peak.aspect} ${peak.natalPlanet}`,
    source: 'solar-arc' as const,
  }));

  const mergedEvents = [...majorHitsOnly, ...solarArcPeaks].sort((left, right) => {
    if (left.year !== right.year) return left.year - right.year;
    const intensityOrder = { strike: 3, burn: 2, shift: 1 };
    return intensityOrder[right.intensity] - intensityOrder[left.intensity];
  });

  console.log(
    `[Life Timeline] Found ${majorHitsOnly.length} transit events and ${solarArcPeaks.length} solar arc peaks`
  );
