...
export interface DualOverlay {
  hardware: DualOverlayLayer;
  firmware: DualOverlayLayer;
  finalType: string;
...
function chartPositions(chart: BirthChartData): PlanetPosition[] {
  return chart.positions ?? chart.planets ?? [];
}

export function buildDualOverlay(chart: BirthChartData, mbtiDual: ReturnType<typeof getMBTIDual>): DualOverlay {
  ...
  const hardwareDesc =
    hardwareDescriptions[hardwareType] ??
  ...
  const firmwareDesc =
    firmwareDescriptions[firmwareType] ??
  ...
  return {
    hardware: {
      ...
      breakdown: mbtiDual.hardware.breakdown,
    },
    firmware: {
      ...
      breakdown: mbtiDual.firmware.breakdown,
    },
...
export function derivePersonalityFromChart(chart: BirthChartData): DerivedPersonality | null {
  ...
  const normalizedChart: BirthChartData = {
    ...chart,
    positions,
