...
import { getClerkPublicMetadata, updateClerkPublicMetadata } from '@/lib/clerk-user-cache';
...
export async function GET() {
  try {
    ...
    const preferences = sanitizePreferences(publicMetadata.oraclePreferences);
...
export async function POST(request: Request) {
  try {
    ...
    const existingMetadata = await getClerkPublicMetadata(userId);
