...
import { validateScorecard } from '@/lib/internal/ops-validators';
...
export async function GET(request: NextRequest) {
  ...
  const rows = await listOpsEvents(OPS_EVENT_TYPES.scorecard, limit);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = validateScorecard(body);
    if (!result.valid) {
      return NextResponse.json(
        { success: false, error: 'Invalid scorecard payload', details: result.errors },
        { status: 400 }
      );
    }

    await appendOpsEvent({
      type: OPS_EVENT_TYPES.scorecard,
      payload: body,
      content: `Scorecard ${String((body as { evaluationWindow?: string }).evaluationWindow || 'unknown-window')}`,
    });

    return NextResponse.json({ success: true, saved: true });
  } catch (error) {
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Failed to save scorecard' },
      { status: 500 }
    );
  }
}
