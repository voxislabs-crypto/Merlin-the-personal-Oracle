...
import { recomputeAtmospherePatterns } from '@/lib/atmosphere/pattern-store.server';
...
export async function POST(request: Request) {
  try {
    ...
    const [result, patternResult] = await Promise.all([
      recomputeCalibrationProfile({
        userId: targetUserId,
        days: body.days,
        minSamples: body.minSamples,
      }),
      recomputeAtmospherePatterns({
        userId: targetUserId,
        days: body.days,
      }),
    ]);
    ...
    await logInteractionEvent({
      ...
      metadata: {
        ...
        patternSamples: {
          feedback: patternResult.feedbackSamples,
          checkins: patternResult.checkinSamples,
        },
      },
    });
