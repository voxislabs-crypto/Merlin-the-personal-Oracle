  ...
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(
        ...
        // Custom solid cards (planet hover) must not inherit translucent primary fill
        hideDefaultChrome && "!bg-transparent !p-0 !text-inherit shadow-none",
        className
      )}
      {...props}
    >
      {children}
      {!hideDefaultChrome ? <TooltipPrimitive.Arrow className="fill-primary" /> : null}
    </TooltipPrimitive.Content>
  </TooltipPrimitive.Portal>
  );
})
