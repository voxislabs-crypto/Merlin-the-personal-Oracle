...
interface WheelProps {
  ...
  onSignSelect?: (name: ZodiacSignName | null) => void;
}

function signWedgePath(cx: number, cy: number, r: number, signIndex: number): string {
  const startDeg = signIndex * 30;
  const endDeg = (signIndex + 1) * 30;
...
export function WheelVisualization({
  ...
  selectedSign: selectedSignProp,
  onPlanetSelect,
  onSignSelect,
}: WheelProps) {
  const [showTransits, setShowTransits] = useState(false);
  const [transitData, setTransitData] = useState<any>(null);
  const [internalSelectedPlanet, setInternalSelectedPlanet] = useState<string | null>(null);
  const [hoveredAspect, setHoveredAspect] = useState<any>(null);

  const isControlled = Boolean(onPlanetSelect);
  const selectedPlanet = isControlled ? (selectedPlanetProp ?? null) : internalSelectedPlanet;
  const selectedSign = selectedSignProp ?? null;

  const handlePlanetClick = (name: string) => {
    const next = selectedPlanet === name ? null : name;
    if (isControlled) {
      onPlanetSelect?.(next);
      if (next) onSignSelect?.(null);
    } else {
      setInternalSelectedPlanet(next);
    }
  };

  const handleSignClick = (name: ZodiacSignName) => {
    const next = selectedSign === name ? null : name;
    onSignSelect?.(next);
    if (next) {
      onPlanetSelect?.(null);
      if (!isControlled) setInternalSelectedPlanet(null);
    }
  };
  ...
  return (
    <div className="flex items-center justify-center w-full h-full bg-black relative">
      ...
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="drop-shadow-2xl z-20 max-w-full max-h-full w-auto h-auto">
        ...
        {/* Zodiac signs — clickable with selection highlight */}
        <motion.g
          animate={{ rotate: -360 }}
        ...
        >
          {ZODIAC_SIGNS.map((sign, i) => {
            ...
            const isDimmed = Boolean(selectedSign && !isSelected);
            ...
            return (
              <g
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handleSignClick(sign.name);
                }}
                role="button"
                aria-label={`${sign.name} sign`}
                aria-pressed={isSelected}
              >
                {isSelected && (
                  <path
                    d={signWedgePath(centerX, centerY, radius * 1.02, i)}
                    fill="#fcd34d"
                    opacity="0.18"
                    stroke="#fcd34d"
                    strokeWidth="2"
                ...
                <circle
                  ...
                  cy={y}
                  r={isSelected ? 34 : 28}
                  fill={isSelected ? '#fcd34d' : 'transparent'}
                  opacity={isSelected ? 0.25 : 0}
                />
                <text
                  x={x}
                  y={y}
                  fill={isSelected ? '#fff7ed' : '#fcd34d'}
                  fontSize={isSelected ? 46 : 42}
                  fontWeight={isSelected ? 700 : 400}
                  textAnchor="middle"
                  dominantBaseline="middle"
                  opacity={isDimmed ? 0.35 : isSelected ? 1 : 0.9}
                  filter={isSelected ? 'url(#goldenGlow)' : undefined}
                >
                  {sign.glyph}
                </text>
              </g>
        ...
        {planets.map((p: any) => {
          ...
          const isDimmed = Boolean(selectedPlanet && !isSelected);
          ...
          return (
            ...
            >
              ...
              <motion.text
                ...
                textAnchor="middle"
                dominantBaseline="middle"
                filter={isHovered || isSelected ? 'url(#goldenGlow)' : ''}
                opacity={isDimmed ? 0.35 : isHovered || isSelected ? 1 : 0.8}
              >
                {glyph}
              </motion.text>
      ...
      {/* Planet Reading Modal — legacy fallback when wheel is not parent-controlled */}
      {!isControlled && selectedPlanet && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setInternalSelectedPlanet(null)}>
          <motion.div
          ...
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <button
                onClick={() => setInternalSelectedPlanet(null)}
